(function($){
    ...
    loadTicketData = function(segment,offset,callback){
        console.log("Load Ticket Data",segment,offset);
        $.ajax({
            url:'/leads/ajaxsource',
            data:{
        ...
        .done(function(res){
            ...
            $.each(res,function(a,b){
                ...
                str+= b.hunter + '<span class="childrenamount"></span>';
                str+= '</td>';
                str+= '<td class="status">';
    ...
    $('#tLeads').on('click','tbody tr .removeTicket',function(){
        tr = $(this).stairUp({level:4});
        clearSelected($('#tLeads tbody tr'),function(){
    ...
    $('#tLeads').on('click','tbody tr .showFUDialog',function(){
        $('#dFollowUp').modal();
    })
